...
export default function HunterSignIn() {
  ...
  const [twoFA, setTwoFA] = useState(null) // { pending_token, message }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      await register({ name: form.name.trim(), email: form.email.trim(), password: form.password, role: 'hunter' })
      navigate(redirect)
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12">
      ...
      <div className="w-full max-w-md">
        <h1 className="text-xl font-semibold text-gray-900 mb-1">Hunter {mode === 'login' ? 'sign in' : 'register'}</h1>
        <p className="text-gray-500 text-sm mb-8">
          {mode === 'login' ? 'Sign in to manage subscriptions and view bookings.' : 'Create an account to subscribe to hunting areas.'}
        </p>
        ...
        <form onSubmit={mode === 'login' ? handleLogin : handleRegister} className="space-y-5">
          ...
          <div>
            ...
            />
            {mode === 'register' && <p className="text-xs text-gray-900/60 mt-1">At least 8 characters</p>}
          </div>
